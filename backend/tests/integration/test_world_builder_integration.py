...
class TestWorldBuilderIntegration:
    ...
    async def test_world_creation_dialogue(
        ...
        # 2. 第一轮对话：用户提出初步想法
        user_input_1 = "我想创建一个叫‘艾瑞多利亚’的奇幻世界，那里有漂浮的岛屿和巨大的水晶森林。"
        
        print(f"\n--- 对话 1 ---")
        print(f"用户输入: {user_input_1}")

        result_1 = await agent.process(
            {"session": session, "user_input": user_input_1}
        )
        ...
        assert "漂浮的岛屿" in session.world_state.geography or "水晶森林" in session.world_state.geography
        ...
        result_2 = await agent.process(
            {"session": session, "user_input": user_input_2}
        )
